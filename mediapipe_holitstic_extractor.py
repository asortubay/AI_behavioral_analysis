...
CROP_PADDING = 0.1  # Padding around person detection for cropping
...
def crop_person_region(frame, bbox, padding=CROP_PADDING):
    """
    ...
    Args:
        ...
        padding: Padding factor (0.1 = 10% padding around bounding box)
    ...
    try:
        ...
        if 'holistic' not in globals() or holistic is None:
            holistic = mp_holistic.Holistic(
                ...
                smooth_segmentation = True, #or enable_segmentation (enable segmentation does not work if we use the changing bounding boxes for the people.)
        ...
        for person_id, bbox in current_people.items():
            try:
                ...
                cropped_frame, crop_coords = crop_person_region(frame, bbox)
